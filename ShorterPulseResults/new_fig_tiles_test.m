...
%%
EOR_store = fields.EOR;
% EOR_store = - fields.EOR / 40 /100;
EOR_store = EOR_store ;
disp(1)
for shift = 0
    EOR = circshift(EOR_store,[0,shift]);
...
%     plot_tile(e_w,t_w, psi_incoherent_or_45);
    ...
    set(gcf,'Position',[200,200,200 + 3 * 300,200 +  300]); %set paper size (does not affect display)
    
    
    exportgraphics(gcf, [setdir,image_name,'shift=',num2str(shift),'.png'], 'Resolution',300);
end
