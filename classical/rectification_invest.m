...
[interact_v_or_phi, interact_v_or_A] = eels.interaction_v(loss_spectrum_parameters);

%%
% alpha_or_0 =  .07;
interact_v_or_store = 1 *  interact_v_or_phi + 1 * interact_v_or_A;
alpha_or_0 = 20 * 1.3 * 1.2;

% interact_v_or = circshift(interact_v_or_store, [18,0]);
...
% loss_spectrum_parameters.interact_v = interact_v_or * 0.07*exp(1i*0);
...
[T, Z, ET, t0_vec, eels_c] = eels_or(electric_field_zt.', tt * 1e12, zz , velec);
% figure;
plot(t0_vec(:)  - 0.05, eels_c/max(eels_c(:))*max(e_exc_or(:)),'r--')
hold off

%%
figure;
s=surf(TT, ZZ, real(electric_field_zt).' / max(real(electric_field_zt(:))));
s.EdgeColor = 'none';
%%
