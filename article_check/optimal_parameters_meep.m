function fields = optimal_parameters_meep(weight, spot_size, shift, TPD_ni, ZPD_ni, EPD_ni, eels_photodember)
...
%     % photodember field calculation
    ...
    [tmeep, zmeep, field_or_meep] = rectification_meep(spot_size_fwhm);
    tmeep = tmeep - delay_or_pd_ps;
    [TOR, ZOR] = ndgrid(tmeep, zmeep);
    EOR = field_or_meep.';
    EOR = EOR / max(EOR(:)) * E_max_rectification * weight;

    %%
%     [TOR, ZOR, EOR] = electric_field_rectification(params_rectification, E_max_rectification, delay_or_pd_ps);
    [T, Z, EPD, EOR] = interpolate_field(TOR, ZOR, EOR, TPD_ni, ZPD_ni, EPD_ni);
