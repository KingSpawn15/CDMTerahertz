function [Phi] = CalcElectricPotential_wXprimeZprimeInt_wRetPotential_womega_xy(PulseEnergy,t,z,x0,y0,gamma,gammaFactor,LaserSpotFWHM,Phase)
...
GaussianLaserSpot(:,:,abs(zprime) > SampleZLength/2,:) = 0;

omega_y = sqrt( (e^2/kappa).*(n_exc.*GaussianLaserSpot.*exp(-alpha.*Yprime).*(1/me0tilda+1/mh)+n_eq/m_eq) - (gamma/2)^2 );
...
parfor TimeInd = 1:length(tCSubSampled)
    ...
    g = (e*alpha*vt^2*n_exc.*GaussianLaserSpot.*exp(-alpha.*Yprime)./(omega_y.*(gamma^2+4.*omega_y.^2))).*...
                        (4.*omega_y.*cos(omega_y.*Tprime + Phase)+2*gamma.*sin(omega_y.*Tprime + Phase)).*...
                        exp(-gamma.*gammaFactor.*Tprime./2);
    g(Tprime<0) = 0;
