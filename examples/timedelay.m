...
interaction_gain_factor_photodember = 0.059;
loss_spectrum_parameters.interact_v = interaction_gain_factor_photodember * v_struct.(strcat('photodember'));
...
loss_spectrum_parameters.interact_v = interaction_gain_factor_photodember * circshift(v_struct.(strcat('photodember')),[-75 0]);
eels = EELS(eels_parameters);
[~ , psi_incoherent_td_1] = eels.energy_loss_spectrum(loss_spectrum_parameters);

loss_spectrum_parameters.interact_v = interaction_gain_factor_photodember * circshift(v_struct.(strcat('photodember')),[75 0]);
eels = EELS(eels_parameters);
[~ , psi_incoherent_td_2] = eels.energy_loss_spectrum(loss_spectrum_parameters);

close all;
figure = tiledlayout(1 , 3 ,'Padding', 'none', 'TileSpacing', 'compact');
nexttile;
imagesc(e_w,t_w, psi_incoherent_td_1);
...
nexttile;
imagesc(e_w,t_w, psi_incoherent_td_2);
