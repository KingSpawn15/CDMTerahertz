...
dir= 'time_analysis/results_efield_map/';
figure;
ttvec = T(1:end,1);
zzvec = Z(1,1:end).';
imagesc(ttvec, zzvec * 1e6, real(ET).' / max(real(ET(:))),[-1,1]);
set(gca,'FontSize',12);
ylim([-120,120]);
xlim([-.3,1.5]);
xticks(-.3:.3:1.5)
...
exportgraphics(gcf, strcat(dir,'emap_rectification_2.png'),'Resolution',500)
% imagesc(ttvec , zzvec, ET);
