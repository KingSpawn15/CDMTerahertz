...
[TPD, ZPD, EPD] = electric_field_photodember(eels_photodember);
...
FontSize = 10;
clim  = max(abs(EOR(:)));
setdir = 'new Figure 1 fields/results/';
...
theta = 90;
[e_w, t_w, psi_incoherent] = generate_incoherent_spectrum_for_angle(EPD, EOR, T, Z, theta, eels_photodember, e_w);
plot_tile(e_w,t_w, psi_assemb_incoherent);
...
function [TPD, ZPD, EPD] = electric_field_photodember(eels_photodember)
    ...
    EPD = utils_spectrum.derivative_f_dz(interact_v_pd, eels_photodember.discretization.z, ...
        eels_photodember.discretization.t);
    EPD(isnan(EPD)) = 0;
