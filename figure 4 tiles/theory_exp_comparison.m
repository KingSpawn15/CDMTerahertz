...
FontName = 'ariel';
FontSize = 10;
plot(com_0, deltat,'LineStyle','--');
plot(com_90 + com_0, deltat,'LineStyle','--');
plot(com_45, deltat,'LineStyle','--');
plot(e_exc_pd,t_w);
xlim([-5,5]);
ylim([-1,1.5]);
set(gca, 'YDir', 'reverse');
axis square;
set_axis_properties(gca,FontSize,FontName,0.5,-1:0.5:1.5,-4:2:4,'','Energy [eV]',FontSize,[0.3 0.3 0.3]);
set(gcf,'Position',[200,200,200 + 300,200 +  300]); %set paper size (does not affect display)
%%
sigma_z = 50e-6;
...
exportgraphics(gcf, ['figure 4 tiles/fig4',num2str(shift_tw),'.png'], 'Resolution',300);
...
%         numerical_parameters] = default_parameters_2(spot_fwhm);
    ...
    if nargin < 1
     ...
     [laser_parameters,discretization_params, utem_parameters,...
        numerical_parameters] = default_parameters_2(40e-6);
